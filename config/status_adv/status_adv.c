...
/* ZMK's split-transport reorg (post-v0.2.1) moved this header and renamed the
 * getter, with an identical signature (uint8_t source, uint8_t *level; 0 = ok):
 *   zmk/split/bluetooth/central.h          -> zmk/split/central.h
 *   zmk_split_get_peripheral_battery_level -> zmk_split_central_get_peripheral_battery_level
 * Support both so this file builds on the v0.2.1 keyboard branches and on the
 * ZMK-main branches (master / charybdis-full). */
#if __has_include(<zmk/split/central.h>)
#include <zmk/split/central.h>
#define STATUS_ADV_GET_PERIPH_BATT zmk_split_central_get_peripheral_battery_level
#else
#include <zmk/split/bluetooth/central.h>
#define STATUS_ADV_GET_PERIPH_BATT zmk_split_get_peripheral_battery_level
#endif
#endif
...
static void build_payload(void) {
...
#if IS_ENABLED(CONFIG_ZMK_SPLIT_BLE_CENTRAL_BATTERY_LEVEL_FETCHING)
    {
        ...
        if (STATUS_ADV_GET_PERIPH_BATT(0, &lvl) == 0) {
            peripheral_batt = lvl;
        } else {
            peripheral_batt = 0;
        }
...
static bool split_ok(void) {
    ...
    {
        uint8_t lvl;
        bool now_up = (STATUS_ADV_GET_PERIPH_BATT(0, &lvl) == 0);
