 ...
 *
 * Upstream: M165437/nice-view-gem, MIT
 *           boards/shields/nice_view_gem/assets/pixel_operator_mono.c
 * The FACE itself is Pixel Operator Mono by Jayvee Enaguas, CC0 -- see below.
 *
 * ONLY LOCAL CHANGE: the exported font was renamed pixel_operator_mono ->
 * dispscan_font_pixel_operator (whole-image namespace, same reason as the
 * glyph files next to it). The generator line below is upstream's, verbatim,
 * and still names the file lv_font_conv actually wrote.
 *
 * WHY THIS FACE AND NOT SOMETHING TALLER: .adv_w is 128 for all 95 glyphs =
 * 8 px advance, exactly the same as lv_font_unscii_8. So the character-counting
 * width arithmetic the layout rests on is unchanged by the swap -- 20 columns
 * across the 160 px panel -- while line_height goes 9 -> 13, buying legibility
 * at the cost of two of the seven text rows. Covers ASCII 0x20..0x7E, the same
 * range unscii_8 does, so the renderer's GLYPH_SUBST sanitising still holds.
 *
 * Size: 16 px
 * Bpp: 1
...
#if LVGL_VERSION_MAJOR >= 8
const lv_font_t dispscan_font_pixel_operator = {
#else
lv_font_t dispscan_font_pixel_operator = {
#endif
